...
class WritingViewModel: ObservableObject {
    ...
    func getTopicList() {
        Task {
            self.loading = true
            let result = try await self.topicRepo().getAllTopic()
            switch (result){
                case is StoryResultSuccess<NSArray>:
                self.topicList = result.data?.map{ ($0 as? Topic)?.name } as? [String] ?? [String]()
                    break
                default:
    ...
    func saveDraft() {
        Task {
            do {
                ...
                try await saveDraftUseCase.invoke(draftId: self.currentDraftId, title: self.title, body: self.content, shouldCreateNewVersion: false)
            } catch{ print(error) }
        }
    }
